...
Before the test are ran, all the language files are re-generated (the current
ones, if any, are backed up and restored after the tests).


Using buildout
--------------

l18n uses zc.buildout_ to generate tests scripts. In (very) short
...
l18n also uses tox_ to test against various environments (mainly python
versions).
...
In each environment, the language files will be re-generated so that the
